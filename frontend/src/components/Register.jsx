...
export default function Register() {
  ...
  const validation = useMemo(() => {
    const issues = {};
    if (!formData.name.trim()) issues.name = "Please enter your full name";
    if (!isValidEmail(formData.email)) issues.email = "Enter a valid email";
    if (!formData.password || formData.password.length < 8)
      issues.password = "Minimum 8 characters";
    return issues;
  }, [formData]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      storage.setItem("authToken", JSON.stringify(result.data.token));
            ...
            <div className="relative rounded-3xl bg-white p-6 shadow-xl ring-1 ring-black/5">
              <div className="mb-6">
                ...
                <p className="mt-1 text-sm text-slate-600">
                  Join us and start organizing your tasks the modern way.
                </p>
              </div>
              ...
              <form noValidate onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    disabled={loading}
                    aria-invalid={submitted && !!validation.name}
        ...
        <div className="order-1 md:order-2">
          <div className="mx-auto flex w-full max-w-md flex-col items-center text-center md:items-start md:text-left">
            ...
            <h2 className="text-4xl font-semibold tracking-tight text-slate-900 md:text-5xl">
              Welcome to your smarter to-do
            </h2>
            <p className="mt-4 max-w-md text-slate-600">
              Organize tasks, focus on priorities, and ship faster with a
              beautifully simple interface.
            </p>

            <div className="mt-8 grid w-full grid-cols-3 gap-3 text-xs text-slate-700">
              <div className="rounded-xl bg-white p-3 shadow-sm ring-1 ring-black/5">
                <p className="font-semibold">One-click capture</p>
                <p className="mt-1 text-slate-500">Add tasks from anywhere.</p>
              </div>
              <div className="rounded-xl bg-white p-3 shadow-sm ring-1 ring-black/5">
                <p className="font-semibold">Smart reminders</p>
                <p className="mt-1 text-slate-500">Never miss a deadline.</p>
              </div>
              <div className="rounded-xl bg-white p-3 shadow-sm ring-1 ring-black/5">
                <p className="font-semibold">Cloud sync</p>
                <p className="mt-1 text-slate-500">All your devices, always.</p>
              </div>
            </div>
          </div>
        </div>
